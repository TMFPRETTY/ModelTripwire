...
def test_regression_gate_and_report_cli(tmp_path: Path) -> None:
    ...
    baseline = run_benchmark_suite(config, project_root, "alpha_core")
    candidate = run_benchmark_suite(config, project_root, "alpha_core")

    config_path = tmp_path / "regression-config.yaml"
...
""".strip(),
    ...
    gate_result = runner.invoke(
        app,
        ["regression-gate", baseline["run"].run_id, candidate["run"].run_id, "--config", str(config_path)],
    )
    assert '"suite_name": "alpha_core"' in gate_result.output
    assert gate_result.exit_code in (0, 1)

    report_dir = tmp_path / "regression_reports"
    ...
    assert "Regression report written to" in report_result.output
